...
def get_trx_type(transaction, user_name):
    ...
    if inbound > 0:
        ...
        if target_name:
            candidates.append(_make_return(types.card_income, target_name))

    # "inbound > 0
    # AND [2]ATM입출금 시트 조건"
    if inbound > 0 and amt_in_out.find(name):
        candidates.append(_make_return(types.atm_in, tr_target_channel='ATM'))
    # "outbound > 0
    # AND [2]ATM입출금 시트 조건"
    if outbound > 0 and amt_in_out.find(name):
        candidates.append(_make_return(types.atm_out, tr_channel='ATM'))
    # "inbound == 1
    # AND [1]계좌인증 조건"
    if inbound == 1:
        finance_vendor = account_auth.find(name)
        if finance_vendor:
            candidates.append(_make_return(types.account_auth, target_name=finance_vendor))
    # 계좌인증 4글자
    if inbound == 1 and len(name) == 4:
        candidates.append(_make_return(types.account_auth))
    # 계좌인증 4글자. 계좌
    if inbound == 1:
        fv = finance_name.find_finance_vendor(name)
        if fv and re.match(korean_4, name):
            candidates.append(_make_return(types.account_auth, target_name=fv))
    # 송금
    # 개인이체 (이름 3,4 자리)
    h_name = human_name.contains(name)
    if h_name and h_name == name and h_name != user_name and '오픈' not in name:
        candidates.append(_make_return(types.send_money, target_name=h_name, target_type='개인'))

    # 개인이체 본인
    if name == user_name and '오픈' not in name:
        candidates.append(_make_return(types.send_money, target_name=user_name, target_type='개인>본인'))

    # 개인이체 (금융사 명칭)
    fv = finance_name.find_finance_vendor(name)
    if h_name and h_name == name and fv and name != user_name and '오픈' not in name:
        candidates.append(_make_return(types.send_money, h_name, '개인', target_account_vendor=fv))

    # 개인이체 (금융사 명칭, 본인)
    if user_name in name and fv:
        candidates.append(_make_return(types.send_money, h_name, '개인>본인', target_account_vendor=fv))

    # 개인이체 (금융사코드)
    fvc = finance_code_transfer.get_finance_vendor(name)
    if fvc and user_name != h_name and '오픈' not in name:
        candidates.append(_make_return(types.send_money, h_name, '개인', target_account_vendor=fvc))

    # 개인이체 (금융사코드, 본인)
    if fvc and user_name == h_name and '오픈' not in name:
        candidates.append(_make_return(types.send_money, user_name, '개인>본인', target_account_vendor=fvc))

    is_toss = toss_transfer.is_toss(name)
    # 간편이체(개인, 토스, 출금)
    if outbound > 0 and is_toss and h_name != user_name:
        candidates.append(_make_return(types.send_money, h_name, '개인', tr_channel='토스'))
    # 간편이체(개인, 토스, 입금)
    ...
    # 간편이체 (본인, 토스, 출금)
    if outbound > 0 and is_toss and h_name == user_name:
        candidates.append(_make_return(types.send_money, user_name, '개인>본인', tr_channel='토스'))
    # 간편이체 (본인, 토스, 입금)
    if inbound > 0 and is_toss  and h_name == user_name:
        candidates.append(_make_return(types.send_money, user_name, '개인>본인', tr_target_channel='토스'))

    # 대출이자
    if outbound > 0 and '까지의 이자를 납입하셨습니다' in name:
        candidates.append(_make_return(types.interest_payment, target_type='금융', target_biz_type='금융 및 보험업'))

    # 카드대금
    card_vendor, card_vendor_biz = card_vendor_name.find_card_vendor_biz_name(name)
    if outbound > 0 and card_vendor:
        candidates.append(_make_return(types.credit_card_payment,
    ...
    if outbound > 0 and fire_ins_name and vendor_type == 'bank':
        candidates.append(
            _make_return(types.ensurance, fire_ins_name, '금융>보험>화재보험', fire_ins_biz_name,
    ...
    if outbound > 0 and yello_umb.match(name):
        candidates.append(
            _make_return(types.yellow_umb, target_name='중소기업중앙회', target_type='공공기관',)
        )

    # 생명보험 보험료 납부
    life_ens_name, life_ens_biz_name = life_ens.find_life_ens(name)
    if outbound > 0 and vendor_type == 'bank' and life_ens_name:
        candidates.append(_make_return(types.ensurance, life_ens_name, '금융>보험>생명보험', life_ens_biz_name, "금융 및 보험업>생명보험업>생명보험업"))

    # 아이템매니아 충전
    if outbound > 0 and user_name in name and 'IMI' in name:
        candidates.append(_make_return(types.recharge_prepaid, target_type='개인>본인', target_account_vendor='아이템매니아'))

    # 카카오뱅크 26주 적금
    if name.startswith('26주적금('):
        candidates.append(_make_return(types.kakao_26_week_saving, target_type='개인', target_account_vendor='카카오뱅크'))

    # 카카오뱅크 자유적금
    if name.startswith('자유적금('):
        candidates.append(_make_return(types.kakao_free_saving, target_type='개인', target_account_vendor='카카오뱅크'))

    # 케이뱅크 적금
    if name.startswith('K뱅') and '적금' in name:
        candidates.append(_make_return(types.saving, target_type='개인', target_account_vendor='케이뱅크'))

    # 예금
    if '예금 [만기일:' in name:
        candidates.append(_make_return(types.deposit, target_type='개인'))

    # 주택 청약
    if outbound > 0 and '주택청약' in name and '회차' in name:
        candidates.append(_make_return(types.housing_subscription, user_name, '개인>본인'))

    # 주택청약(금융사코드)
    fn = finance_code.find_fn(name)
    if fn and '.주택청약' in name:
        candidates.append(_make_return(types.housing_subscription, user_name, '개인>본인', target_account_vendor=fn))

    # 카카오페이 내역 처리
    if '카카오페이' == name:
        candidates.append(_make_return(types.recharge_prepaid, user_name, '개인>본인', target_account_vendor='카카오페이'))

    # 네이버페이 내역 처리
    if '네이버페이' == name:
        candidates.append(_make_return(types.recharge_prepaid, user_name, '개인>본인', target_account_vendor='네이버페이'))
    ...
    # 입금 내역 이체 처리
    if inbound > 0:
        candidates.append(_make_return(types.income))

    # 긴 숫자 이체 처리
    if not name.startswith('010') and number_5gt.match(name):
        candidates.append(_make_return(types.unk))
